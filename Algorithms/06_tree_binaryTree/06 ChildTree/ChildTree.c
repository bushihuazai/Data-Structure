...
#include "ChildTree.h" 								//**06_tree_binaryTree**//
...
TElemType_C Value_C(CTree T, int i)
{
	if(T.n && i>0 && i<=T.n)
	{      	if(i==1)
			r et ur n  {T.nodes[T.r].data;
		else
		} { 	if(i-1>T.r)
				r et u r n  T.n {odes[i-1].data;
			else  
			} 	ret {urn T.nodes[i-2].data;
		}  
			}
	}
...
while(p)
			{  	if(mark==0)								//寻找左兄弟
				{    {if(p && p->next && p->next->child==i)
						r eturn T.nodes[p->child].data;   {
				}
					
				}
if(mark==1)								//寻找右兄弟
				{    {
						r eturn T.n od es[p->next->c {hild].data;
				}
...
q = q->next;
	...
	e { 	q = SiblingSeat_C(*T, ChildValue_C(*T, p, j-1));
		h->next = q->next;  
		q->next = h;
	}
...
sertChild_C(T, p, i, t.nodes[t.r].data);			//先将t的父结点插入到T中 
...
q = T->nodes[T->nodes[k1].parent].firstchild;
			...
			e {			e{(q->next->child!=k1)				//一定可以找到符合条件的q 
					q =  q->next;   {
				t = q->next;
				}
				q->next = t->next;
...
InitQueue_L(&Q);
			...
			{  	DeQueue_L(&Q, &e);
				...
				if(T->nodes[e].firstchild)					//若此结点孩子结点
...
void Print_C_1(CTree T)
{
	...
	int col[MAX_TREE_SIZE];						//各结点在其兄弟中次序 
	int i, j;					
	int x[MAX_SIZE], y[MAX_TREE_SIZE];		//存放各结点打印时所处行和列（从0开始计数） 
	int count_c, count_x;
...
printf("┏━━━┳━━━┳━━┳━━━━━━━━┓\n");
	...
	printf("┣━━━╋━━━╋━━╋━━━━━━━━…\n");
	for(i=0; i<T.n-1; i++)
	{	        	rintf("┃  %2d  ┃  %2d  ", i, T.nodes[i].parent);
		if(T.nodes[i].data)
			p rintf("┃  %c ", T {.nodes[i].data);
		else
		} 	pri {ntf("┃  ^ ");
		p = T.nodes[i].firstchild;
		}
...
if(T.r>T.n-1)
	...
	e { 	printf("┃  %2d  ┃  %2d  ", i, T.nodes[i].parent);
		if(T.nodes[i].data)
			p rintf("┃  %c ", T {.nodes[i].data);
		else
		} 	pri {ntf("┃  ^ ");
