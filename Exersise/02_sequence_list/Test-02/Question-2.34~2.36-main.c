...
XorPointer Prior_Next_Ptr_XorL(Xor L, XorPointer cur, int mark)
{
    ...
    if (!L || !L->Left || !cur) {                //链表 not exsist或链表 is empty或cur is empty则无法求地址
        return NULL;
    }
...
int Length_XorL(Xor L)
{
    ...
    if (!L) {
        exit(OVERFLOW);    //链表 not exsist则退出
    }
...
XorPointer GetElemPtr_XorL(Xor L, int i)    //i为元素位序
{
    ...
    if (!L || !L->Left) {                    //链表 not exsist或链表 is empty无法求地址
        return ERROR;
    }
...
┃(07-1)输出L，mark表示输出方向 ┃
┗━━━━━━━━━━━━━━━*/
/* 方法1：借助Prior_Next_Ptr_XorLFunc遍历链表 */
Status Algo_2_34_1(Xor L, int mark)
{
    XorPointer cur;
    ...
    if (!L || !L->Left) {            //链表 not exsist或链表 is empty则无法输出
        return ERROR;
    }
