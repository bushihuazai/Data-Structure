...
#include "RowLinkSparseMatrix.h" 				//**05_array_lists**//
...
Status CreateSMatrix_RL(FILE *fp, int n, ...)
{
	...
	for (count = 1; count <= n; count++) {
		...
		for (k = 0; k <= MAXRC; ++k) {				//Initial数组rpos
			(*M).rpos[k] = 0;
		}
...
Status AddSMatri_RL(RLSMatrix M, RLSMatrix N, RLSMatrix *Q)
{
	...
	for (i = 0; i <= MAXRC; ++i) {							//Initial数组rpos
		Q->rpos[i] = 0;
	}
...
Status MultSMatrix_RL(RLSMatrix M, RLSMatrix N, RLSMatrix *Q)
{
	...
	if (M.tu * N.tu) {									//Q是非零矩阵
		for (arow = 1; arow <= M.mu; ++arow) {				//处理M的每一行
			//arow为乘积元素在Q中的行号
			for (i = 0; i <= N.nu; ++i) {					//InitialQ中行元素值计数器
				ctemp[i] = 0;
			}
