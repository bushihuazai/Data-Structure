...
#include "SequenceArray.h" 								//**05_array_lists**//
...
Status InitArray(Array *A, int dim, ...)
{
	...
	A->dim = dim;										//Initial数组维度

	A->bounds = (int *)malloc(dim * sizeof(int));			//Initial数组维度信息表
	if (!A->bounds) {
		return OVERFLOW;
	}
	...
	if (!A->base) {									//Initial数组空间
		return OVERFLOW;
	}

	A->constants = (int *)malloc(dim * sizeof(int));		//Initial数组映像Func常量信息表
	if (!A->constants) {
		return OVERFLOW;
	}
