...
void DFS_7_27(ALGraph G, int i, int j, int k);    //通过有限的深度优先遍历判断路径是否exsists
...
int main(int argc, char *argv[])
{
    ...
    mark ? printf("exsists") : printf(" not exsist");
    printf("长度为 %d 的路径!\n", k);
    printf("\n");
    ...
    Algo_7_28(G, i, j);
    if (!path[0][0]) {
        printf("%c 到 %c 之间 not exsist路径：", G.vertices[i].data, G.vertices[j].data);
    } else {
        printf("%c 到 %c 之间exsists %d 条路径：\n", G.vertices[i].data, G.vertices[j].data, path[0][0]);
        printf("路径长    路径\n");
...
/*━━━━━━━━━━━━━━━━━━━━━━━┓
┃题7.27：判断i到j之间是否exsists长度为k的简单路径 ┃
┗━━━━━━━━━━━━━━━━━━━━━━━*/
Status Algo_7_27(ALGraph G, int i, int j, int k)
{
    int v;
    ...
    DFS_7_27(G, i, j, k);                         //通过有限的深度优先遍历判断路径是否exsists
...
Status Algo_7_28(ALGraph G, int i, int j)
{
    ...
    for (m = 1; m <= G.vexnum; m++) {
        visited[m] = FALSE;    //将访问标记Initial为未访问
    }

    InitStack_Sq(&S);                            //Initial路径栈（暂存路径）

    for (m = 0; m <= MAX; m++) {                    //Initial路径数组
