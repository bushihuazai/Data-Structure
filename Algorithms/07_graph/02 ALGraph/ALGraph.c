...
Status DeleteArc_AL(ALGraph *G, VertexType_AL v, VertexType_AL w)
{
	...
	e { 	while(p && p->adjvex!=k2)
		{    	q = p;
			p = p->nextarc;
		}
		
if(!p)
			r etur {n ERROR;								//欲删除的边 not exsist
		else    
		} { 	q->nextarc = p->nextarc;
			free(p);
		}	 
...
for(v=1; v<=G.vexnum; v++)
		vi si t ed[v ]  = FALSE;					/ {/Initial为未访问
